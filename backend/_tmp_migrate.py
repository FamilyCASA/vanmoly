...
for col_name, col_def in cols_to_add:
    try:
        ...
        print(f"ADDED case_study.{col_name}")
    except Exception as e:
        print(f"case_study.{col_name}: {e}")

# 2. Add case_id to customer_workflow
try:
    cur.execute("ALTER TABLE customer_workflow ADD COLUMN case_id INTEGER")
    print("ADDED customer_workflow.case_id")
except Exception as e:
    print(f"customer_workflow.case_id: {e}")

# 3. Create case_workflow_timeline table
cur.execute("""
CREATE TABLE IF NOT EXISTS case_workflow_timeline (
...
# 4. Mark real cases
cur.execute("""
    UPDATE case_study SET is_real_case = 1
    WHERE customer_id IS NOT NULL
      AND building_id IS NOT NULL
      AND customer_id > 0
      AND building_id > 0
""")
print(f"Marked {cur.rowcount} real cases")
