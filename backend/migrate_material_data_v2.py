...
def migrate_data():
    print("=" * 50)
    print("Material Data Migration V2")
    print("=" * 50)
    ...
    try:
        ...
        tgt_cursor.execute("DELETE FROM material_variant")
        tgt_cursor.execute("DELETE FROM material_sku")
        tgt_cursor.execute("DELETE FROM material_category")
        tgt_cursor.execute("DELETE FROM material_supplier")
        tgt_conn.commit()
        ...
        for sku in skus:
            ...
            import json
            try:
                images = json.dumps(json.loads(images)) if images else '[]'
            except:
                images = '[]'
                
            try:
                customization_rules = json.dumps(json.loads(customization_rules)) if customization_rules else '[]'
            except:
                customization_rules = '[]'
                
            try:
                variant_options = json.dumps(json.loads(variant_options)) if variant_options else '[]'
            except:
                variant_options = '[]'
                
            try:
                craft_parts = json.dumps(json.loads(craft_parts)) if craft_parts else '[]'
            except:
                craft_parts = '[]'
        ...
        try:
            ...
            for var in variants:
                ...
                new_sku_id = sku_id_map.get(old_sku_id)
                if new_sku_id:
                    try:
                        variant_values = json.dumps(json.loads(variant_values)) if variant_values else '{}'
                    except:
                        variant_values = '{}'
            ...
            if "no such table" in str(e):
                print("  Source has no variant table, skipping")
            else:
                raise
        print()
        ...
        try:
            ...
            if "no such table" in str(e):
                print("  Source has no supplier table, skipping")
            else:
                raise
        print()
