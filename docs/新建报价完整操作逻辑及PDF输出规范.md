...
- 自定义封面元素：手动设置封面各项视觉及信息元素（贴合后续PDF封面规范），可调整布局、配色，添加必要信息标识。可保存为模板
...
- 关联客户：筛选客户，导入基本信息、项目名称、项目地址、户型信息等核心内容，作为报价关联基础。

- 关联案例：筛选规划师保存的案例（可选），双向关联，案例详情自动关联本报价信息（隐藏单价就是物料配置单）。
...
- 物料筛选与选择：通过物料筛选功能，快速找到所需物料，确认后，添加新的一行，输入自定义商品名称，后面跟随物料名称及物料类别。
- 重要：同一行实现：筛选（筛选房间名称，也可以自定义房间名称）+自定义"商品名称"+筛选物料：通过模糊搜索，下拉菜单（符合关键词的物料名称，包含物料的所有字段，可以输入大类，也可以输入品牌，花色，材料等等关键词）快读定位物料，实现物料筛选功能，自动填充物料名称及物料类别。

- 物料参数录入：必填参数包括数量、单位；可选参数（按需录入）包括定制尺寸（宽度+深度+高度）（计算出计量数据）、工艺要求；补充参数包括型号、规格、材质、品牌、物料主图、备注，确保物料信息精准可追溯。

- 自定义的商品名称放在物料名称前，物料行的显示：该空间的物料编号(D&B-KT-01)+(Z自定义商品名称)+（下拉选择物料类别）+（筛选物料名称）+（引用数据库物料主图）+引用单价+引用单位+（输入定制参数（宽度+深度+高度））+（=公式（计算出定制结果））+（输入数量）+（筛选工艺增项名称：筛选物料库二级分类中”特殊工艺“，物料数据库上传物料的时候，对工艺增项都需要按照备注上传，没有特殊工艺的，后面的值工艺系数默认为1，单价默认为0，数量默认0）+（引用工艺系数）+（输入工艺数量）+（引用工艺单位：例如”米“，“厘米”）+（引用“工艺单价”）+（=公式（计算出金额）；这就完成一行；然后筛选物料新增一行，默认继承上一行的（自定义商品名称）；直到整个空间的物料都添加完成；然后再点击添加空间；直到整个全案的报价全部完成；

- 物料金额计算公式：单价*计量值（默认值1）*数量+工艺单价*工艺系数（默认值1）*工艺数量=物料金额。
...
如果已经用EXCEL表格做了报价表（表格符合标准模板字段规范）：可以直接导入，作为客户的物料清单配置，导入的物料与数据库的物料可能不一致，不一致的物料信息就存入私库（与公库物理隔离），经过管理员修改，确认后可以加入到公库；不然会污染公库物料数据；

### 3\.4 报价表数据库导入

如果这个房间的配置与已经保存的报价表类似，也可以导入已经保存的数据库，按照房间名称查找筛选，选择单行导入，多行导入，该房间全部导入，整个报价表全部导入；导入后，再进行微调，节约大量时间；
...
- 排版规范：每个房间模块独立分隔，表格清晰，图片不拉伸，文字不溢出，确保可读性。标题字段填充淡绿色，人民币单元格用”￥：55\.00“格式；
